function delta=powernp2(alfa,power,h,f)
%%%
%powernp :  Gives the non-centrality parameter for a power function 
%           with (alfa, power, h and f)--Bisection algorithm
%--------------------------------------------------------------------------
%OUTPUT  :
%           delta   = Lower Bound of the Non-Centrality Parameter
%INPUT   :
...
%[1] Aydin C and Demirel H (2004),Computation of Baarda's lower bound
...
%%
%Searching for the solution interval [0 5*10^3],[5*10^3 10^5], [10^5 10^6]
%The capacity for the solution is 10^6
if power<alfa
...
power1=alfa;
power2=powerf(alfa,h,f,5*10^3);

if (power1<=power)&&(power<power2)
    ll=0;lk=5*10^3;eps2=10^-5;
else
    power3=powerf(alfa,h,f,10^5);
    if (power2<=power)&&(power<power3)
        ll=5*10^3;lk=10^5;eps2=10^-4;
    else
        power4=powerf(alfa,h,f,10^6);
        if (power3<=power)&&(power<power4)
            disp('This is very exceptional case. Please wait...')
            ll=10^5;lk=10^6;eps2=10^-3;
        else
...
while ~done
    ...
    if abs(lk-ll)<eps2
        ...
        if abs(1-powerf(alfa,h,f,delta))<eps2
