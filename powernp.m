function np=powernp(alfa,power,h,f)
...
if f>10^12 %this is for saving time. To see whether it works even for f>10^12, remove this if-end
    f=10^12;
end
if power<alfa
...
if delta<1
    delta=log(1-alfa)-log(1-power);
end

while done1==0
    done=0;i=0;logT=log(1-alfa);m=1-alfa;
    sumP=exp(logT-delta);
    ...
    while done==0
        ...
        m=m-c;
        if m>0
            logT=logT+log(delta)+log(m)-log(m+c)-log(i); %log of each term T(i+1)
            P=exp(logT-delta); %each term P(i+1);
            sumP=sumP+P;
    ...
    if delta<0
        delta=LNPapp(alfa,power,h,f,10^-5)/2;
        say=0;
    end
    if say==15
        delta=LNPapp(alfa,power,h,f,10^-5)/2;
...
c=bbo;
 ...
 %%
function delta=LNPapp(alfa,power,h,f,eps2)
